 ...
 */
BarcodeScanner.prototype.getChar = function(event) {
    var code = null;
...
/**
 * Primary event listener. Attach to the keypress listener on the document object.
 * 
 * Currently this listener doesn't set itself, so when setting use jQuery.proxy or
 * Prototype's .bind() to rebind the listener to it's instance, e.g.: 
 * 
 *     var scanner = new BarcodeScanner();
 *     $(document).keypress(jQuery.proxy(scanner.listener, scanner));
 */
BarcodeScanner.prototype.listener = function(event) {
    var keyPressed = this.getChar(event);
